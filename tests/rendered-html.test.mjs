...
test("keeps the growth surfaces present", async () => {
  ...
  assert.match(adminPage, /Esta configuração afeta somente esta filial/);
  ...
  assert.match(adminPage, /Herdar o padrão da empresa/);
  assert.match(adminPage, /Salvar padrão da empresa/);
