...
test("keeps the growth surfaces present", async () => {
  ...
  assert.match(adminPage, /Esta alteração afeta somente/);
  ...
  assert.match(adminPage, /Configuração definida exclusivamente para esta filial/);
  ...
  assert.match(pageStyles, /\.parameter-effective-result/);
  ...
  assert.match(adminPage, /Salvar padrão/);
