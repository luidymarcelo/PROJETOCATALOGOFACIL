...
test("keeps the growth surfaces present", async () => {
  ...
  assert.match(adminPage, /Escolha o destino na finalização/);
  ...
  assert.match(page, /className="order-channel-selector"/);
  assert.match(page, /Enviar pelo WhatsApp/);
  assert.match(page, /onSendOrder\(activeOrderChannel\)/);
  assert.match(pageStyles, /\.order-channel-selector/);
